...
function walking_rotating_from_point(A, B, way, varargin)
...
%Turns positions to the same size as robot and turns degrees to radians
A(1) = A(1)/100;
A(2) = A(2)/100;
A(3) = A(3) *pi/180;
B(1) = B(1)/100;
...
%Defines a matrix for the start position of the robot
...
%Checks if the user choose the py-way or the xy-way. And initialize the 
% one they choose
if way == 'py'
    pythago = 1;
...
for i=1:opt.niterations * 3 %Choose 3 for 'xy' and 5 for 'py'
    ...
    %This is the postion matrix where the rows are the legs and the
    %colloms are x-coordinates, y-coordinates and rotation around z-axis
    %legs = [lx1*cos(rot), ly1*sin(rot), rot;
    %     lx2*cos(rot), ly2*sin(rot), rot;
    %     lx3*cos(rot), ly3*sin(rot), rot+pi;
    %     lx4*cos(rot), ly4*sin(rot), rot+pi]
    
    %For each iteraion the walking function of choose is called upond and
    %this updates the postion of the robot.
    if pythago == 1
    ...
    %Create the legs with the information from the updatet matrix. 
    ...
    %animates the legs wich get saved in a video
...
%The function witch makes the robot rotate around the first leg. 
...
%The function witch makes the robot rotate walk straight forward. 
...
%The initialize for the pytagoras way
...
function [Legs, Rotate, walk] = pytagoras_walk(Legs, L, W, Rotate, first_rot, walk, straight, final_rot, times, i, rotatestart)
    ...
    %final rotation so it matches the same as the input
...
%The function which updates the walking with xy way
function [Legs, Rotate, walk_x, walk_y] = x_then_y_walk(Legs, L, W, Rotate, walk_x, walk_y, x_walk, y_walk,first_rot, second_rot, final_rot, times, i, rotatestart)
    ...
    %First rotate so it walks along the x-axis
