...
classdef PoseGraph < handle
    ...
    methods
        ...
        function world = scanmap(pg, varargin)
            ...
            world = zeros(pg.ngrid, pg.ngrid, 'int32'); %Creates an empty map, with same size as the one scaning
            for i=1:1:pg.graph.n %Goes in steps of one each itterajon, itterates through nodes.
                
                if rem(i, 20) == 0
                    waitbar(i/pg.graph.n, h)
                end
                
                xy = pg.scanxy(i); %scannes around one node and returns cartisian coordinates
                [r,theta] = pg.scan(i); 
                xy(:,r>40) = []; %creates a list with the point closer than 40
                xyt = pg.graph.coord(i); %creates x, y and rotation on the robot
                ...
                for s=1:numcols(xy)
                    % end of each ray
                    [x2,y2] = pg.w2g(xy(:,s));  %points that are an opsticle (closer than 40)
                    
                    % all cells along the ray
                    p = bresenham(x1, y1, x2, y2); %Creates a linje with x and y points for each step in a line
                    try                    
                        k = sub2ind(size(world), p(:,1), p(:,2)); %K is cells witch p runs throu.
                        
                        k1 = k(1:end-1); k2 = k(end); %k1 everything but last element, k2 last element.
                        
                        world(k1) = world(k1) - 1; %free spaces are negativ, 
                        world(k2) = world(k2) + 1; %obsticals or unknown are positive, last element of line.            
                    catch me
           ...
           [numRows,numCols] = size(world); %Check the number of rows and collums in world
            M = 10; %create the M varibel with a size
            for i = 1:numRows %itterate throu each row
                for j = 1:numCols %itterate throu each collum
                    if world(i,j) < -M %Checks if the open spaces have been confirmed more than M times
                        world(i,j) = 0; %If it is confirmed it turns the cell to 0
                    else
                        world(i,j) = 1; %If it dosent confirm it turns the cell to 1
                    end
                end
            end    
        ...
        function plot_occgrid(pg, w)
            ...
            w=-w;
            T = 'Group-8';
            idisp(w, 'nogui','invert', 'title',T, 'xydata', {x, y})
            xlabel('x'); ylabel('y');
        end
